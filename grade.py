...
def print_grade_to_file(dict_grade, dict_activity):
    ...
    for key, value in dict_grade.items():
        # if file exist, clear all data
        file_object.truncate(0)

        # print id into grade-file.txt
        file_object.write(f"ID\t\t: {key}\n")

        # print activity title into grade-file.txt
        file_object.write(f"Title\t: {dict_activity[key]['Title']}\n")

        # print nilai format
        file_object.write("Nilai semua mahasiswa:\n")
        file_object.write("NIM\t\t|Nilai\t\t\n")
        file_object.write("-"*20 + "\n")

        # loop grade nilai
        for nim, nilai in dict_grade[key].items():
            # print nim and nilai into grade-file.txt
            file_object.write(f"{nim}\t\t|{nilai}\t\t\n")
    file_object.close()
