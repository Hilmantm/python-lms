...
def update_submission(dict_submission, list_topic, dict_activity, nim):
    ...
    # Menampilkan seluruh list topic
    for index, value in enumerate(list_topic):
        index += 1
        print(f"{index}. {value['Title']}")

    # Request nomor topic
    choose_topic = int(input("Masukkan nomor topic: "))

    # Tampilkan list assigment berdasarkan nomor topic
    print("List assigment: ")
    print(f"ID\t\t|Title\t\t\t|Type\t\t\t|Description")
    for i in list_topic[choose_topic - 1]['Activities']:
        if dict_activity[i]['Type'] == "assignment":
            print(
                f"{i}\t\t|{dict_activity[i]['Title']}\t\t|{dict_activity[i]['Type']}\t\t|{dict_activity[i]['Description']}")

    # Request id assigment
    choose_assigment = int(input("Masukkan ID assignment: "))

    # Print jawaban sebelumnya
    print("Jawaban Sebelumnya: ")
    jawaban_assignment = dict_submission[choose_assigment][nim]
    print(jawaban_assignment)

    # Request jawaban baru atau kosongkan jika batal update
    jawaban_baru = input("Masukkan jawaban baru atau kosongkan jika batal update: ")
    if jawaban_baru.strip() == "":
        pass
    else:
        dict_submission[choose_assigment][nim] = jawaban_baru

    # Untuk membuktikan bahwa data sudah terupdate, maka print dict_submisstionnya
...
if __name__ == "__main__":
    ...
    dict_mhs = {
        "field": [
            ("Nama", "^([a-zA-Z] ([ '-]| ')?[a-zA-Z] ){1,4}$"),
            ("Email", "^([a-z0-9] [._]?[a-z0-9] ) [@]\w [.]\w{2,3}"),
            ("Password", "^[a-zA-Z0-9]{8,12}$"),
        ],
        "data": {
            "113": {"Nama": "Dummy", "Email": "dummy@telU.com", "Password": "12345678"},
    ...
    list_topic = [
        {
            "Title": "Dummy Topic 1",
            "Description": "Ini deskripsi topic 1",
            "Activities": [0, 1],
        },
        {
            "Title": "Dummy Topic 2",
            "Description": "Ini deskripsi topic 2",
            "Activities": [2],
        },
    ]
    ...
    dict_activity = {
        0: {
            ...
            "Type": "assignment",
            "Description": "buatlah program Game",
        },
        1: {
            "Title": "Dummy material",
            "Type": "material",
            "Description": "slide minggu ini",
        },
        2: {
            "Title": "Dummy Assignment 2",
            "Type": "assignment",
            "Description": "buatlah program LMS",
        },
    }
